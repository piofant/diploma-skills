...
description: Проверка и подготовка диплома/ВКР к прохождению антиплагиата и ИИ-детектора (Антиплагиат.ВУЗ и аналоги). Сканирует текст на признаки GPT-генерации по 10-пунктному чек-листу, выдаёт постраничный риск-репорт, переписывает помеченные куски с конкретикой и неровным ритмом. Опционально — autoresearch-петля против внешнего нейродетектора (метрика → переписать → замерить → оставить если упало → повторить). Использовать когда пользователь говорит "проверь диплом", "пройдёт ли антиплагиат", "ИИ-детектор", "нейродетектор", "вкр на оригинальность", "переделай чтоб не палилось как GPT", "антиплагиат вуза", "0% ИИ", или прикладывает .docx/.pdf диплома. НЕ про оформление текста (см. diploma-format) и НЕ про презентацию защиты (см. diploma-defense).
---
...
**Калибрация (реальные данные):** первый прогон диплома на Антиплагиат.ВУЗ (май 2026) — ИИ-контент **25,52 %**, 20 красных страниц из 47. После переписывания по этому чек-листу + autoresearch-петли против нейродетектора Яндекса — доведено до **0 %**.
...
- Спрашивает про антиплагиат вуза, eLIBRARY, «Кольцо вузов», нейродетектор
...
- Список литературы
- Таблицы (во многих вузах ИСКЛЮЧЕНЫ из проверки — отдельный лайфхак)
...
5. **Прятать прозу в таблицы.** Во многих вузах таблицы исключены из проверки. 5 абзацев требований → 8-строчная таблица.
...
Если есть доступ к внешнему ИИ-детектору с API — превратить переписывание из «на ощущениях» в **измеримую оптимизацию**. Это применение общего скилла `autoresearch` к метрике «% ИИ».

**Чем мерить.** В кейсе автора — нейродетектор Яндекса (доля сгенерированного текста, есть API):
👉 https://ai.yandex.ru/neurodetector — веб-интерфейс для ручной проверки
👉 https://yandex.cloud/ru/docs/foundation-models/ — API Yandex Foundation Models

Подойдёт любой детектор, который отдаёт **число** и доступен программно, — важна не конкретная модель, а возможность замерять после каждой правки. Без API петля вырождается в ручной цикл «скопировал → вставил в форму → посмотрел %», что тоже работает, просто медленнее.
...
5. **Повторять**, пока не достигнут целевой порог (типовой порог ≤ 10 %; в кейсе доведено до 0 %).
...
- ✅ Проза → таблицы (часто вне проверки).
...
## Контекст системы Антиплагиат.ВУЗ (актуально на май 2026)
...
- ИИ-контент: официальный порог вуза **≤ 10 %** (в 2025-2026 жёстко). Для контекста: 25,52 % диплом ещё прошёл в мае 2026, но это риск — целиться сильно ниже.
